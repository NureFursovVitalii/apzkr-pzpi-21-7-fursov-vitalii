...
def training_list(request, year=None, month=None):
    ...
    for day in range(1, days_in_month + 1):
        ...
        trainings = Training.objects.filter(datetime__date=date)
        days_in_calendar.append({'date': date, 'trainings': trainings})

    # Calculate previous and next month
    first_day_of_current_month = current_month.replace(day=1)
    previous_month = first_day_of_current_month - timedelta(days=1)
    next_month = first_day_of_current_month + timedelta(days=days_in_month)
